...
void AMyActor::ChangeColor() {
  if (MeshComp && ColorMaterial) {
    switch (FMath::Rand() % 3) {
      case 0: {
        ColorMaterial->SetVectorParameterValue("Color",
                                               FVector4(1.0f, 0.0f, 0.0f));
        PrintScreenMessage(3.0f, FColor::Red, TEXT("Color Change: Red"));
      } break;
      case 1: {
        ColorMaterial->SetVectorParameterValue("Color",
                                               FVector4(0.0f, 0.0f, 1.0f));
        PrintScreenMessage(3.0f, FColor::Blue, TEXT("Color Change: Blue"));
      } break;
      case 2: {
        ColorMaterial->SetVectorParameterValue("Color",
                                               FVector4(1.0f, 1.0f, 0.0f));
        PrintScreenMessage(3.0f, FColor::Yellow, TEXT("Color Change: Yellow"));
      } break;
      default: {
      } break;
    }
  ...
  if (GetWorld()) {
    if ((PhysicsHandle && MeshComp) && bEnablePhysicsHandle) {
      ...
      if (Controller) {
        ...
        FVector GrabLocaion = ViewLocation + (ViewRotation.Vector() * 1000.0f);
